...
\begin{theorem}
  A {\em $G$-operad} $\O$ is equivalently a list of spaces $\O(T)$ for all $T\in\SetG$, natural in $T$, along with equivariant composition maps $\gamma_F:\O(T)\times\icap_tO(F_t)\to\O(\ico_tF_t)$ for all $F: T\to \Set^G$ constant on orbits, which are natural in $T$ and $F$:
  \begin{itemize}\itemsep-4pt
  \item $\gamma_F(\O(\sigma)x',\icap_tx_t) = \O(\hat\sigma)\gamma_{F\sigma}(x',\icap_sx_{\sigma s})$
  \item $\gamma_{\ico_tcodom(f_t)}(x,\icap_t\O(f_t)(x_t)) = \O(\ico_tf_t)\gamma_F(x,\icap_tx_t)$
  \end{itemize}
for all $F:T\to\SetG$, $\sigma: S\to T$, $x'\in \O(S)$, $x\in\O(T)$, $x_t\in\O(F_t)$, and $f_t: F_t\to F'_t$.\qed
\end{theorem}
...
\todo[inline]{intro stuff, possibly move to appendix}
\cite{cheng_weak_2002}

\subsection{$G$-Symmetric Multicategories}
We define a {\em $G$-symmetric multicategory} to be a symmetric multicategory with category of objects $\O_G^{\scriptsize{\mbox{Iso}}}$, the orbit groupoid of our group.
\begin{remark}
...
\end{remark}
\begin{remark}
  We observe that $\F\O_G = \SetGI$ 
\end{remark}
\todo[inline]{we need to figure out what variance we want $\F X$ to have in terms of $\sigma$; that will determine the $\sigma$ action on $Q(T,G/H)$. }
